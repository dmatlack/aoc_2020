...
fn parse_year(s: &Option<String>) -> usize {
    s.as_ref().unwrap().parse().unwrap_or(0)
}

impl Passport {
    fn from(fields: &Vec<String>) -> Passport {
        let mut passport = Passport::default();
        for field in fields {
            match (field.split(":").nth(0).unwrap(), String::from(field.split(":").nth(1).unwrap())) {
                ("byr", value) => passport.birth_year = Some(value),
                ("iyr", value) => passport.issue_year = Some(value),
                ("eyr", value) => passport.expiration_year = Some(value),
                ("hgt", value) => passport.height = Some(value),
                ("hcl", value) => passport.hair_color = Some(value),
                ("ecl", value) => passport.eye_color = Some(value),
                ("pid", value) => passport.passport_id = Some(value),
                ("cid", value) => passport.country_id = Some(value),
                (_, _) => panic!("Invalid field: {}", field),
            }
        }
        passport
    }
    ...
    fn is_valid_extended(&self) -> bool {
        ...
        let birth_year = parse_year(&self.birth_year);
        if birth_year < 1920 || birth_year > 2002 {
            return false;
        }
        // iyr (Issue Year) - four digits; at least 2010 and at most 2020.
        let issue_year = parse_year(&self.issue_year);
        if issue_year < 2010 || issue_year > 2020 {
            return false;
        }
        // eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
        let expiration_year = parse_year(&self.expiration_year);
        ...
        match re.captures(self.height.as_ref().unwrap()) {
            Some(captures) => {
                let height = u32::from_str(captures.get(1).unwrap().as_str()).unwrap();
        ...
        if let None = re.find(self.hair_color.as_ref().unwrap()) {
            return false;
        }

        // ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
        let re = Regex::new("^(amb|blu|brn|gry|grn|hzl|oth)$").unwrap();
        if let None = re.find(self.eye_color.as_ref().unwrap()) {
            return false;
        }

        // pid (Passport ID) - a nine-digit number, including leading zeroes.
        let re = Regex::new(r"^\d{9}$").unwrap();
        if let None = re.find(self.passport_id.as_ref().unwrap()) {
            return false;
        }
...
pub fn run() {
    println!("Day 4 Part 1: {:?}", one());
    println!("Day 4 Part 2: {:?}", two());
}
