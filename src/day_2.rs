...
fn parse_repeats(s: &str) -> (usize, usize) {
    let repeats: Vec<&str> = s.split("-").collect();
    (usize::from_str(repeats[0]).unwrap(), usize::from_str(repeats[1]).unwrap())
}

fn parse(line: &str) -> Password {
    let tokens: Vec<&str> = line.split(" ").collect();
    assert!(tokens.len() == 3);
    let (min_repeat, max_repeat) = parse_repeats(tokens[0]);
    let character = tokens[1].chars().nth(0).unwrap();
    let password = String::from(tokens[2]);
    Password { min_repeat, max_repeat, character, password }
}
...
fn one() -> u32 {
    ...
    for line in INPUT.split("\n") {
        ...
        if repeat_count >= password.min_repeat && repeat_count <= password.max_repeat {
...
fn two() -> u32 {
    ...
    for line in INPUT.split("\n") {
        ...
        let match_first = chars[password.min_repeat - 1] == password.character;
        let match_second = chars[password.max_repeat - 1] == password.character;
...
pub fn run() {
    println!("Day 2 Part 1: {:?}", one());
    println!("Day 2 Part 2: {:?}", two());
}
